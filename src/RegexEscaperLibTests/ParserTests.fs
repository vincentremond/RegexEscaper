...
let ``Full replacement no match`` () =
    let result =
        substitute "^abcdefghij$" 2 3 ".." (Some "GroupName") "abcdefghij"
...
let ``Full replacement with group`` () =
    ...
    let expected =
        SubstitutionResult.Success { NewRegex = "^a(?<GroupName>.+)efghij$" }
...
let ``Full replacement without group`` () =
    let result =
        substitute "^abcdefghij$" 2 3 ".+" (None) "abcdefghij"

    let expected =
        SubstitutionResult.Success { NewRegex = "^a(.+)efghij$" }
