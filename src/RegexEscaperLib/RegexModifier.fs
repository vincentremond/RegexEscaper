...
module RegexModifier =
    ...
    let internal getRegexParts (originalRegex: string) (startIndex: int) (selectionLength: int) : RegexParseResult =
        if selectionLength = 0 then
            ...
            let selected = originalRegex.Substring(startIndex, selectionLength)

            let after = originalRegex.Substring(startIndex + selectionLength)
    ...
    let private tryGetNewRegex
        ...
        if not matchResult.Success then
            SubstitutionResult.Failure {
                Message = sprintf "Substitution “%s” doesn't match “%s”." replacement unescaped
            }
        else
            ...
            let newRegex = sprintf "%s%s%s" parseResult.Before group parseResult.After
            ...
            if Regex.IsMatch(sampleValue, newRegex) then
                ...
                SubstitutionResult.Failure {
                    Message =
                        sprintf
                            "Hum something unexpected happened, the final value does not match\nSample value : %s\nNew regex : %s"
                            sampleValue
                            newRegex
                }
    ...
    let substitute
        ...
        (sampleValue: string)
        : SubstitutionResult =
        let parsed = getRegexParts originalRegex startIndex selectionLength
